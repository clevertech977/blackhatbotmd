...
sock.ev.on('messages.upsert', async ({ messages }) => {
  for (const msg of messages) {
    ...
    // ✅ Automatic chatbot reply
    if (chatbotCmd && chatbotCmd.handleMessage) {
      await chatbotCmd.handleMessage(sock, msg);
    }
...
// GOD-level Anti-Delete integration
sock.ev.on('messages.delete', async ({ key }) => {
  try {
    if (!antideleteCmd || !antideleteCmd.handleDeletedMessage) return;

    await antideleteCmd.handleDeletedMessage(sock, key);
...
sock.ev.on('presence.update', async (update) => {
  try {
    const botUser = db.getUser('bot');
    if (!botUser?.autoStatus) return; // only if enabled

    // WhatsApp stories (statuses) are in update.presences
    // Example: automatically mark status as viewed
    for (const jid in update) {
      const presence = update[jid];
      if (presence.lastSeen && presence.status) {
        // Use WhatsApp API to mark as read (pseudo-code)
        // sock.viewStatus(jid); // Implement this according to Baileys
        console.log(`Viewed status of ${jid}`);
      }
    }
...
// Example: in your main message listener
sock.ev.on('messages.upsert', async ({ messages }) => {
  for (const msg of messages) {
    if (!msg.message) continue;

    const groupId = msg.key.remoteJid;
    const senderId = msg.sender || msg.key.participant;

    // 1️⃣ Handle silenced users
    const settings = database.getGroupSettings(groupId);
    const silencedUsers = settings.silencedUsers || {};

    if (silencedUsers[senderId]) {
      if (Date.now() < silencedUsers[senderId]) {
        await sock.sendMessage(groupId, { delete: msg.key });
        continue; // Skip command
      } else {
        delete silencedUsers[senderId];
        database.updateGroupSettings(groupId, { silencedUsers });
      }
    }
    ...
    const extra = {
      ...
      from: groupId,
      sender: senderId,
      reply: async (text) => sock.sendMessage(groupId, { text, quoted: msg }),
      replyId: msg.key.participant || msg.sender
    };
